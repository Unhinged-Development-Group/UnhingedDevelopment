...
def main():
    ...
    # ── mainland (index 0) + islands ───────────────────────────────────
    mainland = subpaths[0]
    islands = subpaths[1:]

    # Drop islands whose starts fall outside the viewBox (Shetland/Orkney)
    margin = 2.0
    def in_viewbox(sp):
        ax, ay = sp['start']
        return (vb_x - margin <= ax + tx <= vb_x + vb_w + margin and
                vb_y - margin <= ay + ty <= vb_y + vb_h + margin)

    visible = [s for s in islands if in_viewbox(s)]
    dropped = len(islands) - len(visible)
    print(f"Islands: keeping {len(visible)}, dropping {dropped} (out of viewBox)")
    ...
    ordered = [mainland] + visible

    # ── verify M coords after transform are in viewBox ─────────────────
    bad = 0
    for sp in ordered:
        ax, ay = sp['start']
        if not in_viewbox(sp):
            bad += 1
    if bad:
        print(f"WARNING: {bad} subpaths still outside viewBox after filtering")
    ...
    ts = (
        ...
        f'// {len(ordered)} subpaths: mainland first, then {len(visible)} islands clockwise.\n'
        f'\n'

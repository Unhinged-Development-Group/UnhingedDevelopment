...
export default function AccountSetup() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session?.user) {
        const uc = session.user.user_metadata?.company ?? "";
        setCompany(uc === "all" ? "unhinged-development" : uc);
        setReady(true);
      }
    });

    // Handle already-authenticated case (e.g. page reload)
      ...
      <div className="flex flex-1 items-center justify-center px-4 pb-20">
        <div className="w-full max-w-sm">
          <div className="mb-8">
            ...
            <p className="mt-1 text-sm text-zinc-500">Choose a password to secure your account before continuing.</p>
          </div>
